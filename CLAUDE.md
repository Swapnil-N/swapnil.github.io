...
- **Dark mode is the site default**: Site pages are dark only. Client demos can opt into light by wrapping `{children}` in `<div data-theme="light">` in their `layout.tsx` (the scaffolder generates this when `--theme=light` is passed). The `[data-theme="light"]` block in `globals.css` overrides the color CSS vars — Tailwind tokens cascade automatically.
...
- **Scaffold script**: `npm run new-demo -- --slug=acme --name="Acme Bakery" [--theme=light]` creates `src/app/demo/{slug}/{layout.tsx, page.tsx, CLAUDE.md}` and `public/demo/{slug}/`. The per-demo `CLAUDE.md` is the source of truth for vibe-coding agents working in that folder (Claude Code auto-loads it). Does NOT touch the DB — create the row via `/admin/demos` first (the form has a Theme selector that emits the right scaffold command).
