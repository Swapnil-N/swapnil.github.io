...
# 2. Vibe-code the demo in src/app/demo/acme-bakery/ (a per-demo CLAUDE.md is
#    dropped in by the scaffolder so any Claude agent picks up the guardrails,
#    including the available theming options).
...
Each demo lives entirely in `src/app/demo/{slug}/` + `public/demo/{slug}/` so it can be cleanly cut and pasted into a graduation repo. The shared `gateDemo()` wrapper in `src/lib/demo/gate.ts` enforces auth and ownership; admins can preview any demo. Demos have full theming flexibility — inline Tailwind colors, opt into the shared light palette via `<div data-theme="light">`, co-located CSS modules, or custom CSS vars on a wrapper.
